...
class DiseaseOntologyTermSerializer(serializers.ModelSerializer):
    ...
    description = serializers.CharField(
        source="ontology_term.description", allow_null=True, required=False
    )  # the description is optional
    source = serializers.CharField(
        source="ontology_term.source.name", required=True
    )  # external source (ex: OMIM)
...
class DiseaseDetailSerializer(DiseaseSerializer):
    ...
    def get_last_updated(self, id: int) -> Optional[date]:
        ...
        filtered_lgd_list = LocusGenotypeDisease.objects.filter(
            disease=id, is_deleted=0, date_review__isnull=False
        ...
        for lgd_obj in lgd_objects_list:
            if lgd_obj["stable_id__stable_id"] not in aggregated_data.keys():
                ...
                if (
                    ...
                    not in aggregated_data[lgd_obj["stable_id__stable_id"]]["panels"]
                ):
                    aggregated_data[lgd_obj["stable_id__stable_id"]]["panels"].append(
                        lgd_obj["lgdpanel__panel__name"]
                    )
                if (
                    lgd_obj["lgdvariantgenccconsequence__variant_consequence__term"]
