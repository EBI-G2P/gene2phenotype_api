...
class UserSerializer(serializers.ModelSerializer):
    ...
    def get_panels(self, id):
        ...
        if user_login and user_login.is_authenticated:
            user_panels = (
                UserPanel.objects.filter(user=id, is_deleted=0)
                .select_related("panel")
                .values_list("panel__description", flat=True)
            )
        else:
            user_panels = (
                UserPanel.objects.filter(user=id, is_deleted=0, panel__is_visible=1)
                .select_related("panel")
                .values_list("panel__description", flat=True)
            )
            ...
            for panel in panels:
                ...
                try:
                    UserPanel.objects.get(
                        user=user_login.id, panel__name=panel_name, is_deleted=0
                    )
...
class CreateUserSerializer(serializers.ModelSerializer):
    ...
    )  # write only because it is not a readable field
    ...
    def get_user_panels(self, obj):
        ...
        """
        return list(
            UserPanel.objects.filter(user=obj, is_deleted=False).values_list(
    ...
    def create(self, validated_data):
        ...
        panels = validated_data.pop(
            "panels", []
        )  # popping panels because i do not need it for creation
...
class AddUserToPanelSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        for panel in panels:
            ...
            if UserPanel.objects.filter(
                user=user_obj, panel=panel_obj, is_deleted=1
            ).exists():
                self.update(user_obj, panel_obj)
                continue

            if not UserPanel.objects.filter(
                user=user_obj, panel=panel_obj, is_deleted=0
            ).exists():
                user_panel = UserPanel.objects.create(
                    user=user_obj, panel=panel_obj, is_deleted=0
                )
            else:
                raise serializers.ValidationError(
        ...
        if username and password:
            ...
            if user.is_deleted:
                raise AuthenticationFailed(
                    "Account disabled. Please contact Admin at g2p-help@ebi.ac.uk"
                )
