...
class CurationDataSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if (
            "locus" not in data_dict["json_data"]
            or data_dict["json_data"]["locus"] == ""
        ...
        if (
            ...
            and len(data_dict["json_data"]["panels"]) >= 1
        ):
            ...
            unauthorized_panels = [
                panel
                for panel in data_dict["json_data"]["panels"]
    ...
    def validate_to_publish(self, data):
        ...
        if (
            "allelic_requirement" not in json_data
            or json_data["allelic_requirement"] == ""
        ):
            missing_data.append("allelic_requirement")

        if (
            "molecular_mechanism" not in json_data
            or json_data["molecular_mechanism"]["name"] == ""
        ):
            missing_data.append("molecular_mechanism")

        if (
            "variant_consequences" not in json_data
            or not json_data["variant_consequences"]
        ):
        ...
        if (
            (
                "autosomal" in json_data["allelic_requirement"]
                and not locus_chr.isdigit()
            )
            or (
                json_data["allelic_requirement"] == "mitochondrial"
                and locus_chr != "MT"
            )
            or (
                "_PAR" in json_data["allelic_requirement"]
                and (locus_chr != "X" and locus_chr != "Y")
            )
            or ("_X" in json_data["allelic_requirement"] and locus_chr != "X")
            or ("_Y" in json_data["allelic_requirement"] and locus_chr != "Y")
        ):
            raise serializers.ValidationError(
                {
    ...
    def publish(self, data):
        ...
        if "cross_references" in data.json_data["disease"]:
            for cr in data.json_data["disease"]["cross_references"]:
                ontology_term = {
                    ...
                    ],  # TODO This should be the full description
                    "source": cr["source"],
                }
        ...
        try:
            ...
            for ontology in cross_references:
                try:
                    ...
                    disease_ontology_serializer = DiseaseOntologyTermSerializer(
        ...
        try:
            mechanism_obj = CVMolecularMechanism.objects.get(
                value=mechanism_name, type="mechanism"
            )
        ...
        try:
            mechanism_support_obj = CVMolecularMechanism.objects.get(
                value=mechanism_support, type="support"
            )
        except CVMolecularMechanism.DoesNotExist:
            raise serializers.ValidationError(
        ...
        lgd_data = {
            ...
            "allelic_requirement": data.json_data[
                "allelic_requirement"
            ],  # value string
        ...
        for m_synopsis in data.json_data["mechanism_synopsis"]:
            if (
                "name" in m_synopsis
                and "support" in m_synopsis
                and m_synopsis["name"] != ""
                and m_synopsis["support"] != ""
            ):
                try:
            ...
            for hpo in hpo_terms:
                ...
                try:
                    lgd_phenotype_serializer = LGDPhenotypeSerializer(
                        data=phenotype_data, context={"lgd": lgd_obj}
                    )
                    # Validate the input data
        ...
        for var_consequence in data.json_data["variant_consequences"]:
            try:
                lgd_var_cons_serializer = LGDVariantGenCCConsequenceSerializer(
                    data=var_consequence, context={"lgd": lgd_obj}
                )
        ...
        for variant_type_desc in data.json_data["variant_descriptions"]:
            LGDVariantTypeDescriptionSerializer(context={"lgd": lgd_obj}).create(
                variant_type_desc
            )

        # Comments
        if (
            "public_comment" in data.json_data
            and data.json_data["public_comment"] != ""
        ):
            comment_obj_public = {
        ...
        if (
            "private_comment" in data.json_data
            and data.json_data["private_comment"] != ""
        ):
            comment_obj_private = {
        ...
        G2PStableIDSerializer(
            context={"stable_id": data.stable_id.stable_id}
        ).update_g2p_id_status(1)
